...
const ProjectCard = ({ project, featured = false }: ProjectCardProps) => {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    console.log(`Card ${project.id} clicked, current state: ${isExpanded}`);
    setIsExpanded(!isExpanded);
  };

  return (
    <div className="group relative bg-white dark:bg-dark-900 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden cursor-pointer flex flex-col h-full mobile-project-card">
      <motion.div
        whileHover={{ y: -8 }}
